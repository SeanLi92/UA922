"use strict";      //author of orginal javascript is Codehim                  
let cart = [];
let cartTotal = 0;
...
const IsinCart = cart.filter(cartItem => cartItem.name === product.name).length > 0; //sets up the cart when add to cart is clicked for an item, adds smaller image of product, name, and price along with buttons to increase and decrease the amount and remove the item from cart
if (IsinCart === false) {
  ...
  <div class="d-flex flex-row shadow-sm card cart-items mt-2 mb-3 animated flipInX">
    ...
    <div class="p-2 mt-3">
        <p class="text-warning cart_item_name">${product.name}</p>
    </div>
    <div class="p-2 mt-3">
    ...
    //creates the clear cart button to empty cart
...
//creates a paypal form that doesn't actually work
function checkOut() {
  let paypalHTMLForm = `
